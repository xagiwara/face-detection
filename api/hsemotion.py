...
def hsemotion (imgs: list[cv2.typing.MatLike], model: str, cuda: str):
    ...
    return [{
        'emotion': emotion[j],
        'scores': { model.idx_to_class[i]: scores[j][i].item() for i in range(len(model.idx_to_class)) }
    } for j in range(len(imgs))]

@app.post('/hsemotion')
async def hsemotion_process(file: UploadFile, cuda: str = Query('cpu', enum=cuda_devices()), model: str = Query(hsemotion_models()[0], enum=hsemotion_models())):
...
async def hsemotion_prepare (cuda: str = Query('cpu', enum=cuda_devices()), model: str = Query(hsemotion_models()[0], enum=hsemotion_models())):
    start = perf_counter()
