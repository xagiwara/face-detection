...
def param2vert(param, dense=False, transform=True):
    ...
    if dense:
        vertex = (
            ...
            @ (
                param_pack.u
                + param_pack.w_shp @ alpha_shp
                + param_pack.w_exp @ alpha_exp
            ).reshape(3, -1, order="F")
            + offset
        )
        if transform:
        ...
        vertex = (
            ...
            @ (
                param_pack.u_base
                + param_pack.w_shp_base @ alpha_shp
                + param_pack.w_exp_base @ alpha_exp
            ).reshape(3, -1, order="F")
            + offset
        )
        if transform:
...
def draw_landmarks(img, pts, wfp):
    ...
    for i in range(len(pts)):
        ...
        for ind in range(len(nums) - 1):
            ...
            plt.plot(
                pts[i][0, l:r], pts[i][1, l:r], color=color, lw=lw, alpha=alpha - 0.1
            )
