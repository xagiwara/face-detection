...
from lib.synergynet.backbone_nets import resnet_backbone
from lib.synergynet.backbone_nets import mobilenetv1_backbone
from lib.synergynet.backbone_nets import mobilenetv2_backbone
from lib.synergynet.backbone_nets import ghostnet_backbone
from lib.synergynet.backbone_nets.pointnet_backbone import MLP_for, MLP_rev
from .loss_definition import ParamLoss, WingLoss

from lib.synergynet.backbone_nets.ResNeSt import resnest50
...
# from lib.synergynet.FaceBoxes import FaceBoxes
import cv2
import types
...
class I2P(nn.Module):
    def __init__(self, args):
        ...
        if "mobilenet_v2" in self.args.arch:
            ...
            raise RuntimeError(
                "Please choose [mobilenet_v2, mobilenet_1, resnet50, or ghostnet]"
            )
...
class SynergyNet(nn.Module):
    def __init__(self, args):
        ...
        self.triangles = param_pack.tri["tri"] - 1
        ...
        self.register_buffer(
            "param_mean", torch.Tensor(param_pack.param_mean).cuda(non_blocking=True)
        )
        self.register_buffer(
            "param_std", torch.Tensor(param_pack.param_std).cuda(non_blocking=True)
        )
        self.register_buffer(
            "w_shp", torch.Tensor(param_pack.w_shp).cuda(non_blocking=True)
        )
        self.register_buffer("u", torch.Tensor(param_pack.u).cuda(non_blocking=True))
        self.register_buffer(
            "w_exp", torch.Tensor(param_pack.w_exp).cuda(non_blocking=True)
        )
        ...
        self.register_buffer(
            "u_base", torch.Tensor(param_pack.u_base).cuda(non_blocking=True)
        )
        self.register_buffer(
            "w_shp_base", torch.Tensor(param_pack.w_shp_base).cuda(non_blocking=True)
        )
        self.register_buffer(
            "w_exp_base", torch.Tensor(param_pack.w_exp_base).cuda(non_blocking=True)
        )
    ...
    def forward(self, input, target):
        ...
        self.loss["loss_LMK_f0"] = 0.05 * self.LMKLoss_3D(
            vertex_lmk, vertex_GT_lmk, kp=True
        )
        self.loss["loss_Param_In"] = 0.02 * self.ParamLoss(_3D_attr, _3D_attr_GT)

        point_residual = self.forwardDirection(
            vertex_lmk, avgpool, _3D_attr[:, 12:52], _3D_attr[:, 52:62]
        )
        vertex_lmk = vertex_lmk + 0.05 * point_residual
        self.loss["loss_LMK_pointNet"] = 0.05 * self.LMKLoss_3D(
            vertex_lmk, vertex_GT_lmk, kp=True
        )

        _3D_attr_S2 = self.reverseDirection(vertex_lmk)
        self.loss["loss_Param_S2"] = 0.02 * self.ParamLoss(
            _3D_attr_S2, _3D_attr_GT, mode="only_3dmm"
        )
        self.loss["loss_Param_S1S2"] = 0.001 * self.ParamLoss(
            _3D_attr_S2, _3D_attr, mode="only_3dmm"
        )
